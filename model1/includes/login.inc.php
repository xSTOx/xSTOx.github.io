...
if (isset($_POST['login-submit'])) {
  ...
  if (empty($mailuid) || empty($password)) {
    header("Location: ../about_me.php?error=emptyfields&mailuid=".$mailuid);
    exit();
  }
  else {
    ...
    if (!mysqli_stmt_prepare($stmt, $sql)) {
      ...
      header("Location: ../about_me.php?error=sqlerror");
      exit();
    }
    else {
      ...
      if ($row = mysqli_fetch_assoc($result)) {
        ...
        if ($pwdCheck == false) {
          // If there is an error we send the user back to the signup page.
          header("Location: ../about_me.php?error=wrongpwd");
          exit();
        }
        // Then if they DO match, then we know it is the correct user that is trying to log in!
          ...
          header("Location: ../about_me.php?login=success");
          exit();
        }
      }
      else {
        header("Location: ../about_me.php?login=wronguidpwd");
        exit();
      }
    }
  }
